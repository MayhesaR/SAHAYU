...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Card 1: Hari Ini -->
        <div class="bg-white p-6 rounded-[1.5rem] border border-stone-200/60 shadow-sm relative overflow-hidden group">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-5 bg-white p-6 sm:p-8 rounded-[1.5rem] border border-stone-200/60 shadow-sm space-y-6">
            <div class="space-y-1">
            ...
            <form action="{{ route('expenses.store') }}" method="POST" class="space-y-4">
                ...
                <!-- Tanggal -->
                <div class="space-y-2">
                    <label for="expense_date" class="block text-[10px] font-black uppercase tracking-wider text-stone-450">Tanggal Pengeluaran</label>
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-4 top-3.5 text-stone-400 text-sm">calendar_today</span>
                        <input type="date" 
                               id="expense_date" 
                               name="expense_date" 
                               value="{{ old('expense_date', date('Y-m-d')) }}" 
                               required 
                               class="w-full bg-slate-50/70 border border-stone-200/60 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-semibold text-slate-800 focus:bg-white focus:border-[#0b6e4f] focus:ring-4 focus:ring-[#0b6e4f]/10 transition-all outline-none" />
                    </div>
                    @error('expense_date')
                ...
                <!-- Kategori -->
                <div class="space-y-2">
                    <label for="category" class="block text-[10px] font-black uppercase tracking-wider text-stone-450">Kategori Pengeluaran</label>
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-4 top-3.5 text-stone-400 text-sm">category</span>
                        <select id="category" 
                                name="category" 
                                required
                                class="w-full bg-slate-50/70 border border-stone-200/60 rounded-2xl pl-12 pr-10 py-3.5 text-sm font-semibold text-slate-800 focus:bg-white focus:border-[#0b6e4f] focus:ring-4 focus:ring-[#0b6e4f]/10 transition-all outline-none appearance-none cursor-pointer">
                ...
                <!-- Nominal -->
                <div class="space-y-2">
                    <label for="amount" class="block text-[10px] font-black uppercase tracking-wider text-stone-450">Nominal Pengeluaran (Rupiah)</label>
                    <div class="relative">
                        <span class="absolute left-4 top-3.5 text-stone-400 text-sm font-black">Rp</span>
                        <input type="number" 
                               id="amount" 
                               name="amount" 
                ...
                <!-- Keterangan -->
                <div class="space-y-2">
                    <label for="description" class="block text-[10px] font-black uppercase tracking-wider text-stone-450">Keterangan / Deskripsi (Opsional)</label>
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-4 top-3.5 text-stone-400 text-sm">description</span>
                        <textarea id="description" 
                                  name="description" 
                                  rows="3"
                                  placeholder="Tulis detail keperluan pengeluaran..." 
                                  class="w-full bg-slate-50/70 border border-stone-200/60 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-semibold text-slate-800 focus:bg-white focus:border-[#0b6e4f] focus:ring-4 focus:ring-[#0b6e4f]/10 transition-all outline-none">{{ old('description') }}</textarea>
                    </div>
                    @error('description')
                        <p class="text-rose-600 text-[10px] font-bold">{{ $message }}</p>
                    @enderror
                ...
                <!-- Submit Button -->
                <button type="submit" 
                        class="w-full py-4 text-white hover:bg-[#09523b] font-bold text-sm rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#0b6e4f]/10 bg-[#0b6e4f]">
        ...
        <div class="lg:col-span-7 bg-white rounded-[1.5rem] border border-stone-200/60 shadow-sm overflow-hidden flex flex-col">
            
            <!-- Table Header -->
            <div class="px-8 py-6 border-b border-stone-200/60">
            ...
            <div class="px-8 py-4 bg-stone-50/40 border-b border-stone-200/60 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <form action="{{ route('expenses.index') }}" method="GET" class="flex flex-wrap items-center gap-3 flex-1">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="date" 
                               ...
                               class="bg-white border border-stone-200/60 rounded-xl px-3 py-2 text-xs font-semibold text-slate-700 outline-none focus:border-[#0b6e4f] transition-all" />
                    ...
                    <button type="submit" 
                            class="px-4 py-2 bg-[#0b6e4f] hover:bg-[#09523b] text-white rounded-xl text-xs font-bold transition-all shadow-sm">
                        Cari
                    </button>
                    @if(request('start_date') || request('end_date') || (request('category') && request('category') !== 'all') || request('sort_by') !== 'newest')
                        <a href="{{ route('expenses.index') }}" 
                           class="px-4 py-2 bg-stone-100 hover:bg-stone-200 text-slate-600 rounded-xl text-xs font-bold transition-all">
                            Reset
                        </a>
                    @endif
                </form>
                ...
                <a href="{{ route('expenses.export', request()->all()) }}" 
                   class="px-4 py-2 bg-[#0b6e4f] hover:bg-[#09523b] text-white rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm">
            ...
            <div class="overflow-x-auto min-w-full">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-stone-200/60 bg-stone-50/40">
            ...
            @if($expenses->hasPages())
                <div class="px-8 py-6 border-t border-stone-200/60 bg-stone-50/40 mt-auto">
                    {{ $expenses->links() }}
                </div>
            @endif
        </div>

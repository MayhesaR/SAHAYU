...
@section('content')
<div class="px-4 py-6 sm:px-8 space-y-6 bg-slate-50/50 dark:bg-zinc-850/30 dark:bg-zinc-800/30 min-h-screen">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Card 1: Total Omzet -->
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-100 dark:border-zinc-800/60 shadow-sm relative overflow-hidden group">
            <div class="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center mb-4">
                <span class="material-symbols-outlined text-xl">payments</span>
            </div>
            <p class="text-[10px] font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Total Omzet Penjualan</p>
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1">Rp {{ number_format($totalSalesAmount, 0, ',', '.') }}</h3>
            <p class="text-[10px] text-slate-400 dark:text-zinc-400 font-medium mt-4">Akumulasi seluruh transaksi penjualan</p>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-100 dark:border-zinc-800/60 shadow-sm relative overflow-hidden group">
            ...
            <p class="text-[10px] font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Transaksi Penjualan</p>
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1">{{ number_format($totalSalesCount, 0, ',', '.') }} <span class="text-sm text-slate-400 dark:text-zinc-400">Transaksi</span></h3>
            <p class="text-[10px] text-slate-400 dark:text-zinc-400 font-medium mt-4">Jumlah nota penjualan yang dicatat</p>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-slate-100 dark:border-zinc-800/60 shadow-sm relative overflow-hidden group">
            ...
            <p class="text-[10px] font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Operasi Produksi</p>
            <h3 class="text-2xl font-black text-slate-900 dark:text-white mt-1">{{ number_format($totalProductionCount, 0, ',', '.') }} <span class="text-sm text-slate-400 dark:text-zinc-400">Batch</span></h3>
            <p class="text-[10px] text-slate-400 dark:text-zinc-400 font-medium mt-4">Jumlah batch produksi barang jadi</p>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('history.index') }}" class="space-y-4">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div class="relative flex-1">
                    <span class="material-symbols-outlined absolute left-4 top-3.5 text-slate-400 dark:text-zinc-400">search</span>
                    <input type="text" 
                           name="search" 
                           value="{{ $search }}" 
                           placeholder="Cari Pelanggan, No. Transaksi, atau Keterangan..." 
                           class="w-full bg-slate-50/70 dark:bg-zinc-850/70 dark:bg-zinc-800/70 border-2 border-slate-100 dark:border-zinc-800/60 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none" />
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
                    ...
                    <button type="button" 
                            ...
                            class="flex-1 sm:flex-none px-4 py-3 bg-emerald-50 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 border border-emerald-200 font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm">
                    ...
                    <button type="button" 
                            ...
                            class="flex-1 sm:flex-none px-4 py-3 bg-slate-50 dark:bg-zinc-850 dark:bg-zinc-800 border border-slate-100 dark:border-zinc-800/60 hover:bg-slate-100 text-slate-700 dark:text-zinc-50 font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm">
            ...
            <div x-show="showAdvanced" 
                ...
                <div class="space-y-2">
                    <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-400">Jenis Transaksi</label>
                    <select name="type" 
                            class="w-full bg-slate-50 dark:bg-zinc-850 dark:bg-zinc-800 border-2 border-slate-100 dark:border-zinc-800/60 rounded-xl p-3 text-xs font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-emerald-600 transition-all outline-none">
                ...
                <!-- Filter 2: Sort By -->
                <div class="space-y-2">
                    <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-400">Urutkan Berdasarkan</label>
                    <select name="sort_by" 
                            class="w-full bg-slate-50 dark:bg-zinc-850 dark:bg-zinc-800 border-2 border-slate-100 dark:border-zinc-800/60 rounded-xl p-3 text-xs font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-emerald-600 transition-all outline-none">
                ...
                <div class="space-y-2">
                    <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-400">Tanggal Mulai</label>
                    <input type="date" 
                           name="start_date" 
                           value="{{ $startDate }}" 
                           class="w-full bg-slate-50 dark:bg-zinc-850 dark:bg-zinc-800 border-2 border-slate-100 dark:border-zinc-800/60 rounded-xl p-3 text-xs font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-emerald-600 transition-all outline-none" />
                ...
                <div class="space-y-2">
                    <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-400">Tanggal Akhir</label>
                    <input type="date" 
                           name="end_date" 
                           value="{{ $endDate }}" 
                           class="w-full bg-slate-50 dark:bg-zinc-850 dark:bg-zinc-800 border-2 border-slate-100 dark:border-zinc-800/60 rounded-xl p-3 text-xs font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-emerald-600 transition-all outline-none" />
    ...
    <div id="tour-history-table" class="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-100 dark:border-zinc-800/60 shadow-sm overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-slate-100 dark:border-zinc-800/60 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="space-y-1">
                ...
                <p class="text-xs text-slate-400 dark:text-zinc-400">Log operasional terpadu yang memvisualisasikan seluruh arus kas masuk, keluar, dan produksi.</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2 bg-slate-100/60 dark:bg-zinc-800/60 p-1.5 rounded-2xl border border-slate-200/40 dark:border-zinc-800/40 w-fit">
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'all'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'all' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Semua
                </a>
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'sale_cash'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'sale_cash' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Penjualan Tunai
                </a>
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'sale_debt'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'sale_debt' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Penjualan Piutang
                </a>
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'payment'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'payment' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Cicilan Piutang
                </a>
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'production'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'production' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Produksi
                </a>
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'expense'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'expense' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Operasional
                </a>
                <a href="{{ route('history.index', array_merge(request()->query(), ['type' => 'purchase'])) }}" 
                   class="px-4 py-2 rounded-xl text-xs font-bold transition-all {{ $currentType === 'purchase' ? 'bg-primary text-white shadow-sm' : 'text-slate-500 dark:text-zinc-400 hover:bg-white/50' }}">
                    Belanja Bahan
                </a>
            </div>
        </div>
        ...
        <div class="overflow-x-auto min-w-full">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-zinc-800/60 bg-slate-50/70 dark:bg-zinc-850/70 dark:bg-zinc-800/70">
                        <th class="px-8 py-4 text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Waktu Transaksi</th>
                        <th class="px-6 py-4 text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Tipe</th>
                        <th class="px-6 py-4 text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Detail Transaksi</th>
                        <th class="px-6 py-4 text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Jumlah / Nominal</th>
                        <th class="px-6 py-4 text-xs font-black text-slate-400 dark:text-zinc-400 uppercase tracking-widest">Status</th>
                    </tr>
                </thead>
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-400 dark:text-zinc-400 font-semibold mt-0.5">{{ $log['details'] }}</p>
                                    </div>
                                </div>
                            </td>
                            <!-- Column 4: Jumlah / Nominal -->
                            <td class="px-6 py-5">
                                <span class="text-sm font-black {{ ($log['type'] === 'expense' || $log['type'] === 'purchase') ? 'text-rose-600' : (($log['type'] === 'production') ? 'text-slate-500 dark:text-zinc-400 font-bold' : 'text-slate-900 dark:text-white') }} bg-slate-100/50 dark:bg-zinc-800/50 px-3 py-1.5 rounded-xl border border-slate-200/20 dark:border-zinc-800/20 font-mono">
                                    {{ $log['amount'] }}
                                </span>
                            </td>
                            <!-- Column 5: Status -->
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center text-slate-400 dark:text-zinc-400">
                                ...
                                <p class="text-xs text-slate-400 dark:text-zinc-400 mt-1">Belum ada aktivitas tercatat pada kategori atau filter ini.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <!-- Pagination Footer -->
        @if($logs->hasPages())
            <div class="px-8 py-6 border-t border-slate-100 dark:border-zinc-800/60 bg-slate-50/50 dark:bg-zinc-850/30 dark:bg-zinc-800/30">

...
<form action="{{ $action }}" method="GET" id="{{ $p }}table-controls-form"
    ...
    <div class="mt-3 pt-3 border-t border-slate-100 dark:border-zinc-800/60 flex items-center gap-2 flex-wrap">
        ...
        @endif
        @if(request($p . 'sort'))
        <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 text-[10px] font-bold rounded-full border border-indigo-200">
            Urutan: {{ request($p . 'sort') }}
        </span>
        @endif
        @if(request($p . 'start_date'))
        <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 text-[10px] font-bold rounded-full border border-blue-200">
        ...
        @foreach($filterOptions as $filter)
            ...
            <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 text-[10px] font-bold rounded-full border border-amber-200">
